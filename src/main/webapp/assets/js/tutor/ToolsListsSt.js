...
console.log(arregloTabla);
    ...
    for (let i = inicio; i < fin && i < datos.length; i++) {
      ...
      filaHTML.innerHTML = `
        <tr class="ancho-columna">
                                  ...
                                  <td>
                                      <form action="${url}/tutor/view-student" method="get">
                                          <input hidden value="${fila.Id}" name="id"/>
                                          <input hidden value="${fila.IdU}" name="idU"/>
                                          <button type="submit" class="btn-green">Ver</button>
                                      </form>
                                  </td>
        </tr>
      `;
  ...
  function filtrarPorNombre() {
    ...
    const datosFiltrados = arregloTabla.filter((fila) =>
      fila.Matricula.toLowerCase().includes(inputFiltro)
    );
