...
for (var i = 1; i < tabla.rows.length; i++) {
      ...
      filaHTML.innerHTML = `
        <tr class="ancho-columna">
                                  ...
                                  <td>
                                          ...
                                          <button type="submit" class="btn-green bForm" value="${fila.Id}" >Seleccionar</button>
  ...
  function filtrarPorNombre() {
    ...
    const datosFiltrados = arregloTabla.filter((fila) =>
      fila.Email.toLowerCase().includes(inputFiltro)
    );
...
let bForm = document.querySelectorAll(".bForm")

bForm.forEach((element, index) => {
  element.addEventListener("click",function(e){
    document.getElementById("id_consultor").value =element.value;
    e.preventDefault();
    pregunta()
  });
  function pregunta() {
    if (confirm('¿Seguro que quieres la tranferencia?')) {
      document.getElementById('formCon').submit();
    }
  }
});